...
const tools = [{
  ...
  function: {
    name: "search_web",
    description: "Webを検索して最新の情報を調べる。あなた自身の知識は2024年頃で止まっているので、知らない固有名詞（新しいモデル名・製品名など）や「最新」「今」を含む話題は、記憶で答えずに必ず先にこれで調べること。比較を頼まれたら、比較対象もこれで調べる。出典つきの答えが返る。同じ語の綴り違いを繰り返し試さないこと（2回調べて見つからなければ、そこで打ち切り、推測で書かずにユーザーに綴りや意味を確認する）。",
    parameters: {
      type: "object",
      properties: {
